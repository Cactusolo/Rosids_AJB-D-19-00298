...
fit.multi.rpanda <- function(tree, Order, file) {
  ...
  saveRDS(result, file=paste0("Cucurbitaceae_5g_family_level/fam_40/RPANDA_fam_40/", Order, "_fam_40.rds", sep=""))
  
}

#################running_panel#####################

# dir.create("Cucurbitaceae_5g_family_level/fam_40/RPANDA_fam_40")
...
for (i in 1:length(files)){
  ...
  tryCatch({
    ...
    Order <- substr(basename(files[i]), 1, nchar(basename(files[i]))-7)
    file <- paste0("Cucurbitaceae_5g_family_level/fam_40/RPANDA_fam_40/", Order, "_RPADNA_model_fam_40.csv", sep="")
