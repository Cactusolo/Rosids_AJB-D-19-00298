...
fit.multi.rpanda <- function(tree, Order, file) {
  ...
  saveRDS(result, file=paste0("fitbd/", Order, ".rds", sep=""))
...
files <- list.files("generated_tree/", pattern = ".tre", full.names = TRUE)

for (i in 1:length(files)){
  
  tryCatch({
    
    Order <- substr(basename(files[i]), 1, nchar(basename(files[i]))-9)
    file <- paste0("fitbd/", Order, "_RPADNA_model.csv", sep="")
