...
# dir.create("genus_level_10DR")
file1 <- "genus_level_10DR/MeanDR_for_each_tree.csv"
cat("tree,MeanDRrate\n", file = file1)
...
for (i in 1:length(files)) {
  ...
  file=paste0("genus_level_10DR/", gsub(".tre", "", basename(files[i])), "_DR.csv", sep="")
  ...
  write.table(paste0(gsub(".tre", "", basename(files[i])), ",", DR2, sep=""), quote = FALSE, file=file1,  row.names=FALSE, col.names=FALSE, append=TRUE)
