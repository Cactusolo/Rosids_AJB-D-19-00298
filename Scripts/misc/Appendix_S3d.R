...
rateylab <- expression(paste("Rate (","mya"^"-1", ")"))
...
for (i in 1:length(orderlist)){
  ...
  plot(BD$median.rate ~ rev(BD$Time), type="l", lwd=2, xlab="Time before present (mya)", ylab="", ylim=yrang, xlim=xrang, bty="n", col=color[i])
