...
library("tidyverse")
#comparison of RTT curves from 10%, 30% 50% and 75% random drop and addin treatment 
#with original tree 
pdf("../Figure/Fig. 7. BAMM_40_RTT_curves_from_treatment_of_dropaddin.pdf", width=6, height=6)
...
files <- list.files("Cucurbitaceae_5g_family_level/Fam_40_random_drop_addin/BAMM_fam_40/result", 
...
for(i in 1:length(files)){
  ...
  if(q==10){
    ...
    legend(15, 2.9, legend=c("Original tree", "10% treatment"), col=c("black", color[brks %in% q]), cex=0.6, lty=1, bty = "n")
  }else if(q==30){
    if(i-10==1){
      # plot.new()
      plot(data$median.rate ~ rev(data$Time), type="l", xlab="Time before present (Myr)", 
    ...
    lines(fam$median.rate ~ rev(fam$Time), type="l", lwd=2, col="black")
    legend(15, 2.9, legend=c("Original tree", "30% treatment"), col=c("black", color[brks %in% q]), cex=0.6, lty=1, bty = "n")
  }else if(q==50){
    if(i-20==1){
      # plot.new()
      plot(data$median.rate ~ rev(data$Time), type="l", xlab="Time before present (Myr)", 
    ...
    lines(fam$median.rate ~ rev(fam$Time), type="l", lwd=2, col="black")
    legend(15, 2.9, legend=c("Original tree", "50% treatment"), col=c("black", color[brks %in% q]), cex=0.6, lty=1, bty = "n")
  }else{
    if(i-30==1){
      # plot.new()
      plot(data$median.rate ~ rev(data$Time), type="l", xlab="Time before present (Myr)", 
    ...
    lines(fam$median.rate ~ rev(fam$Time), type="l", lwd=2, col="black")
    legend(15, 2.9, legend=c("Original tree", "75% treatment"), col=c("black", color[brks %in% q]), cex=0.6, lty=1, bty = "n")
  }
}
dev.off()