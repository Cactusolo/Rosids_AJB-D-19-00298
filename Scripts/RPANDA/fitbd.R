...
fit.multi.rpanda <- function(tree, Order, file) {
  ...
  fraction <- length(tree$tip.label)/length(Total.tree$tip.label)
  ...
  write.table(paste0(Order, ",", model, ",", bcst.d0$lamb_par[1], ",", bcst.d0$aicc, sep=""), file, row.names=F, col.names=F, append=T)
  ...
  write.table(paste0(Order, ",", model, ",", bcst.dcst$lamb_par[1], ",", bcst.dcst$aicc, sep=""), file, row.names=F, col.names=F, append=T)
  ...
  write.table(paste0(Order, ",", model, ",", bvar.d0$lamb_par[1], ",", bvar.d0$aicc, sep=""), file, row.names=F, col.names=F, append=T)
  ...
  write.table(paste0(Order, ",", model, ",", bvar.l.d0$lamb_par[1], ",", bvar.l.d0$aicc, sep=""), file, row.names=F, col.names=F, append=T)
  ...
  write.table(paste0(Order, ",", model, ",", bvar.dcst$lamb_par[1], ",", bvar.dcst$aicc, sep=""), file, row.names=F, col.names=F, append=T)
  ...
  write.table(paste0(Order, ",", model, ",", bvar.l.dcst$lamb_par[1], ",", bvar.l.dcst$aicc, sep=""), file, row.names=F, col.names=F, append=T)
  ...
  write.table(paste0(Order, ",", model, ",", bcst.dvar$lamb_par[1], ",", bcst.dvar$aicc, sep=""), file, row.names=F, col.names=F, append=T)
  ...
  write.table(paste0(Order, ",", model, ",", bcst.dvar.l$lamb_par[1], ",", bcst.dvar.l$aicc, sep=""), file, row.names=F, col.names=F, append=T)
  ...
  write.table(paste0(Order, ",", model, ",", bvar.dvar$lamb_par[1], ",", bvar.dvar$aicc, sep=""), file, row.names=F, col.names=F, append=T)
  ...
  saveRDS(result, file=paste0(newname, "_", Order, ".rds", sep=""))
...
Rpanda_go <- function(dataset){
  
  file <- paste0("result/RPANDA_model_", dataset, ".txt", sep="")
  cat("Clade,Model,lamda,AICc", file = file)
 ...
 for (i in 1:length(order.list)){
  ...
  tryCatch({
    ...
    #read tree
    tree <- read.tree(paste("data/", dataset, "/", Order, "_", dataset, ".tre", sep=""))
...
####run whole trees####
#Note: 4g=9k, 5g=20k, and otl=100k
...
tree.list <- c("4g", "5g", "otl")
Rpanda_go2 <- function(Order){
  
  file <- paste0("whole_tree/RPANDA_model_", Order, ".txt", sep="")
  cat("Clade,Model,lamda,AICc", file = file)
   tryCatch({
     #read tree
     tree <- read.tree(paste("whole_tree/rosids_", Order, ".tre", sep=""))
     if(is.ultrametric(tree)){
       print("TRUE")
     }else{
       tree <- force.ultrametric(tree, method="extend")
...
# Vitales <- readRDS("./result/RPANDA_model_4g_Vitales.rds")
# 
# for (i in 1:9){
#   print(Vitales[[i]]$lamb_par)
# }